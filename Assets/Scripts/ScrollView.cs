...
namespace UnityEngine.UI
{
    ...
    public abstract class ScrollView : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        private List<GameObject> m_CacheItems = new List<GameObject>();
        private static GameObject m_CacheItemsRoot;
            ...
            get
            {
                ...
                if (content != null)
                {
                    ...
                    if (layout2 != null)
                    {
                        ...
                        {
                            Debug.LogWarning("[ScrollRect] Flexible not supported yet");
                        }
        ...
        protected override void Awake()
        {
            ...
            if(itemPrefab && itemPrefab.transform.parent == content)
            {
                ReturnItem(itemPrefab.transform);
            }
        ...
        public void SrollTo(int index, float speed)
        {
            ...
            StopAllCoroutines();
            StartCoroutine(ScrollToCoroutine(index, speed));
        }

        IEnumerator ScrollToCoroutine(int index, float speed)
        {
        ...
        public void Refresh()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        OnScrollItem(content.GetChild(i), itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
                ...
                while ((reverseDirection ? NewItemAtStart() : NewItemAtEnd()) > 0) { }
               
            }
        }

        private void OnScrollItem(Transform item,int index)
        {
            item.SendMessage("OnScrollItem", index, SendMessageOptions.DontRequireReceiver);
            if(onScrollItem!=null)
            {
                onScrollItem.Invoke(item, index);
            }
        }


        public void RefillFromEnd(int offset = 0)
        {
            if (!Application.isPlaying || itemPrefab == null)
                return;
            ...
            {
                ReturnItem(m_Content.GetChild(i));
            }
        ...
        public void Refill(int offset = 0)
        {
            if (!Application.isPlaying || itemPrefab == null)
                return;
            ...
            // Don't `Canvas.ForceUpdateCanvases();` here, or it will new/delete items to change itemTypeStart/End
            for (int i = m_Content.childCount - 1; i >= 0; i--)
            {
                ReturnItem(m_Content.GetChild(i));
            }
        ...
        private RectTransform InstantiateNextItem(int itemIdx)
        {
            ...
            while (m_CacheItems.Count > 0)
            {
                if (m_CacheItems[0] != null)
                {
                    nextItem = m_CacheItems[0].transform as RectTransform;
                    m_CacheItems.RemoveAt(0);
                    break;
                }
                else
                {
            ...
            if (nextItem == null)
            {
                nextItem = Instantiate(itemPrefab).transform as RectTransform;
            }
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            OnScrollItem(nextItem, itemIdx);
            return nextItem;
        }
        ...
        private void ReturnItem(Transform item)
        {
            ...
            if(m_CacheItemsRoot == null)
            {
                m_CacheItemsRoot = new GameObject("ScrollViewCacheItemsRoot",typeof(RectTransform));
                m_CacheItemsRoot.transform.position = new Vector3(99999, 99999, 99999);
                DontDestroyOnLoad(m_CacheItemsRoot);
            }
        ...
        {
            base.OnDestroy();
            for (int i = 0; i < m_CacheItems.Count; ++i)
            {
                DestroyImmediate(m_CacheItems[i]);
            }
            m_CacheItems.Clear();
        }
